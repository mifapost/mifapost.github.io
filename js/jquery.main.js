...
function initMobileNav() {
	jQuery('html').mobileNav({
		...
		onInit: function(self) {
			self.drop.find('.nav-close').on('click', function(e) {
				e.preventDefault();
				self.hide();
			});
			self.drop.find('#nav').on('click', function() {
				self.hide();
			});
		}
	});
...
function initTimer() {
	...
	var countDownDate = new Date("Apr 29, 2018 00:00:00").getTime();
...
function initGoogleMaps() {
	...
	function init() {
			...
			var mapOptions = {
					...
					center: new google.maps.LatLng(49.987288, 36.224472), // New York
			...
			var marker = new google.maps.Marker({
					position: new google.maps.LatLng(49.987288, 36.224472),
					map: map
			});
	}
}
...
function initAnchors() {
	...
	ResponsiveHelper.addRange({
		'1024...': {
			on: function() {
				anchorInstance2 = new SmoothScroll({
					anchorLinks: 'a[href^="#"]:not([href="#"])',
					extraOffset: 100,
					activeClasses: 'link',
					sectionActiveClass: '',
		...
		refreshActiveClass: function() {
			...
			$.each(this.anchorData, function(index) {
				...
				if (scrollTop >= containerHeight - viewPortHeight) {
					...
					if (!foundFlag && (scrollTop >= data.offset.top - 1 - win.height() /2 || reverseIndex === 0)) {
						foundFlag = true;
						self.toggleActiveClass(anchorElement, data.block, true);
					} else {
						self.toggleActiveClass(anchorElement, data.block, false);
